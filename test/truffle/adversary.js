...
contract('Offers test', async (accounts) => {
    ...
    async function createEscrow(id, fromAccount, escrowCreationGas) {
        ...
        await adversaryInstance.createEscrow(id, {
            value: ethForOracle,
    ...
    async function claimEscrow(id, fromAccount, escrowClaimGas) {
        ...
        await adversaryInstance.claimEscrow(id, {
            value: ethForOracle,
            from: accounts[fromAccount],
            gasPrice: gasPriceToUse
        });
    ...
    it("Claim escrow", async function () {
        var expectedError = false;
        try {
            await adversaryInstance.finalise({from: accounts[0]});
        }
        catch (err) {
            expectedError = true;
        }
        if (!expectedError) {
            throw "destroyed contract before OK";
        }

        await adversaryInstance.disableContract();
        expectedError = false;
        try {
            await adversaryInstance.createOffer(true, 'ETHUSD', 10 * oneDai, 2, 0, 0, 0, {from: accounts[0]});
        }
        ...
        if (!expectedError) {
            throw "created escrow after disabling contract";
        }
